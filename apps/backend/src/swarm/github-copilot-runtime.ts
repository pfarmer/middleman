...
const GITHUB_COPILOT_RUNTIME_STATE_ENTRY_TYPE = "swarm_github_copilot_runtime_state";
const GITHUB_COPILOT_SANDBOX_MODE = "danger-full-access";
...
interface GithubCopilotRuntimeState {
  threadId: string;
}

interface GithubCopilotSandboxSettings {
  sandboxMode: typeof GITHUB_COPILOT_SANDBOX_MODE;
  threadConfig: {
    sandbox_mode: typeof GITHUB_COPILOT_SANDBOX_MODE;
  };
  turnSandboxPolicy: {
...
export class GithubCopilotRuntime implements SwarmAgentRuntime {
  ...
  private readonly sandboxSettings: GithubCopilotSandboxSettings;
  ...
  private constructor(options: {
    ...
    this.sandboxSettings = buildGithubCopilotSandboxSettings();

    const command = process.env.GITHUB_COPILOT_BIN?.trim() || "github-copilot-agent";
    ...
    this.rpc = new CodexJsonRpcClient({
      ...
      onStderr: () => {
        // Intentionally ignored. GitHub Copilot agent emits debug logs on stderr.
      }
    });
  }
  ...
  static async create(options: {
    ...
    const runtime = new GithubCopilotRuntime(options);
    ...
    try {
      ...
      const normalized = normalizeGithubCopilotStartupError(error);
  ...
  private async ensureAuth(): Promise<void> {
    ...
    if (!account.requiresAuth || account.account) {
      return;
    }

    const githubToken = process.env.GITHUB_TOKEN?.trim() || process.env.GITHUB_COPILOT_TOKEN?.trim();
    if (githubToken) {
    ...
    if (!refreshed.requiresAuth || refreshed.account) {
      return;
    }

    throw new Error(
      "GitHub Copilot runtime requires authentication. Set GITHUB_TOKEN in Settings."
    );
  }

  private async readAccount(): Promise<{ requiresAuth: boolean; account: unknown }> {
  ...
  private async bootstrapThread(): Promise<void> {
    ...
    if (!startedThreadId) {
      throw new Error("GitHub Copilot runtime did not return a thread id");
    }
  ...
  private readPersistedRuntimeState(): GithubCopilotRuntimeState | undefined {
    const entries = this.getCustomEntries(GITHUB_COPILOT_RUNTIME_STATE_ENTRY_TYPE);
  ...
  private persistRuntimeState(): void {
    ...
    this.appendCustomEntry(GITHUB_COPILOT_RUNTIME_STATE_ENTRY_TYPE, {
      threadId: this.threadId
    });
  }
  ...
  private async startTurn(message: RuntimeUserMessage): Promise<void> {
    ...
    if (!this.threadId) {
      throw new Error("GitHub Copilot runtime thread is not initialized");
    }
    ...
    try {
      const response = await this.rpc.request<{ turn?: { id?: unknown } }>("turn/start", {
        ...
        input: toGithubCopilotInputItems(message)
      });
    ...
    while (this.queuedSteers.length > 0 && this.activeTurnId) {
      ...
      try {
        await this.rpc.request("turn/steer", {
          ...
          input: toGithubCopilotInputItems(queued.message)
        });
...
// GitHub Copilot agent defaults new threads to read-only sandboxing.
// We use danger-full-access so agents have unrestricted filesystem access.
function buildGithubCopilotSandboxSettings(): GithubCopilotSandboxSettings {
  return {
    sandboxMode: GITHUB_COPILOT_SANDBOX_MODE,
    threadConfig: {
      sandbox_mode: GITHUB_COPILOT_SANDBOX_MODE
    },
    turnSandboxPolicy: {
      type: "dangerFullAccess"
    }
...
function normalizeGithubCopilotStartupError(error: unknown): Error {
  if (isSpawnEnoentError(error)) {
    return new Error(
      "GitHub Copilot agent is not installed or not available on PATH. Install github-copilot-agent or set GITHUB_COPILOT_BIN."
    );
  }
...
function toGithubCopilotInputItems(message: RuntimeUserMessage): unknown[] {
  const items: unknown[] = [];
